...
async function computeNDCI(bboxStr, months, env, polygonGeoJson, opts = {}) {
  ...
  if (result.n_intervals_used === 0) {
    // 0.9.7: bbox-tilassa evalscript maskaa maan JA pilvet noDataksi, joten
    // maavaltaisella bboxilla (vettä < 50 %) yksikään väli ei läpäise
    // validiosuusrajaa säästä riippumatta. Kerrotaan syy suoraan.
    return {
      error: "no_valid_intervals", n_intervals_total: result.n_intervals_total,
      time_range: { from, to }, resolution_m: resolutionM,
      ...(polygonGeoJson ? {} : {
        hint: "NDCI bbox-tilassa vaatii, että vettä on vähintään 50 % bboxista (maa ja pilvet maskataan samalla tavalla, niitä ei voi erottaa). Maavaltaiselle alueelle käytä polygonia: ?polygon=iisvesi (−40 m, kalibroitu perusarvo) tai ?polygon=iisvesi_raw. Tarkista vesiosuus /mndwi-reitin water_fraction_pct-kentästä."
      })
    };
  }
